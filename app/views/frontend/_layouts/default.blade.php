...
<html>
    ...
    <body>
        ...
        <div class="l-wrap">
            @if ( $errors )
                @if ( is_array($errors) )
                    <div class="alert alert-danger alert-dismissible" role="alert" style="position: absolute; width: 100%; z-index: 10;">
                        <button type="button" class="close" data-dismiss="alert"><span aria-hidden="true">&times;</span><span class="sr-only">Close</span></button>
                        @foreach($errors['notifications'] as $error)
        ...
        <script>
            ...
            window.siteBaseUrl         = {{ "'".URL::to('/')."'" }};
            @if ( $hotel->hotel_slug )
                window.selectedHotelSlug   = {{ "'".$hotel->hotel_slug."'" }};
            @else
                window.selectedHotelSlug   = {{ "'undefined'" }};
            @endif

            @if ( $hotel->hotel_slug )
                window.selectedPackageSlug = {{ "'".$hotel->package_slug."'" }};
            @else
                window.selectedPackageSlug = {{ "'undefined'" }};
            @endif
        </script>
