			...
			<div class="col-sm-12">
				<h1 class="page-header">
					...
					<div class="pull-right">
						<a href="{{ URL::route('backend.discounts.create') }}" class="btn btn-primary"><i class="glyphicon glyphicon-plus"></i> Add</a>
					</div>
				</h1>
			</div>
        </div>
		<div class="row">
			<div class="col-sm-12">
				<div class="table-responsive">
                    <table class="table table-striped table-bordered table-checkable" id="tableDiscounts">
                        <thead>
                        <tr>
                            <th>
                        ...
                        <tr>
                            ...
                            <td><input type="text" name="search_code" value="" class="form-control input-sm" placeholder="Search" autocomplete="off" /></td>
                            <td><input type="text" name="search_count" value="" class="form-control input-sm" placeholder="Search" autocomplete="off" /></td>
                            <td><input type="text" name="search_created_at" value="" class="form-control input-sm" placeholder="Search" autocomplete="off" /></td>
                            <td><input type="text" name="search_expire" value="" class="form-control input-sm" placeholder="Search" autocomplete="off" /></td>
...
<script type="text/javascript">
    ...
    $(document).ready(function() {
        table = $('#tableDiscounts').dataTable( {
            "sDom": "<'row dt-rt'<'col-sm-6'l>r>t<'row dt-rb'<'col-sm-6'i><'col-sm-6'p>>",
            "bProcessing": true,
            "bServerSide": true,
            "oLanguage": { "sSearch": "Search:" },
            "sAjaxSource": "{{ URL::route('backend.discounts.datatables') }}",
            "bSortCellsTop": true,
            "iDisplayLength": 50,
            "aaSorting": [
                [6,'desc']
...
//            ]
    ...
    function deleteItem(id) {
        if (confirm('Are you sure you want to delete this discount?')) {
            $.ajax({
                url: "{{ URL::route('backend.discounts.destroy', '') }}/" + id,
                type: "DELETE",
    ...
    function activateSelected() {
        ...
        if (ids.length > 0) {
            $('#formBulkActions').attr('action', "{{ URL::action('App\Controllers\Backend\DiscountsController@activate') }}");
    ...
    function deactivateSelected() {
        ...
        if (ids.length > 0) {
            $('#formBulkActions').attr('action', "{{ URL::action('App\Controllers\Backend\DiscountsController@deactivate') }}");
            $('#formBulkActions').find('input[type="hidden"]').val(ids);
            $('#formBulkActions').submit();
        }
    }
