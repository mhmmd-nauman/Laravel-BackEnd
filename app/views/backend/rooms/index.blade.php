			...
			<div class="col-sm-12">
				<h1 class="page-header">
					...
					<div class="pull-right">
						<a href="{{ URL::route('backend.rooms.create') }}" class="btn btn-primary"><i class="glyphicon glyphicon-plus"></i> Add</a>
					</div>
				</h1>
			</div>
        </div>
		<div class="row">
			<div class="col-sm-12">
				<div class="table-responsive">
					<table class="table table-striped table-bordered table-checkable" id="tableRooms">
						<thead>
							<tr>
                                <th>
							...
							<tr>
                                ...
                                <td><input type="text" name="search_max_residents" value="" class="form-control input-sm" placeholder="Search" autocomplete="off" /></td>
...
<script type="text/javascript">
	...
	$(document).ready(function() {
		table = $('#tableRooms').dataTable( {
			...
			"sAjaxSource": "{{ URL::route('backend.rooms.datatables') }}",
			"bSortCellsTop": true,
			"iDisplayLength": 50,
			"aaSorting": [
				[6,'desc']
	...
	function deleteItem(id) {
		if (confirm('Are you sure you want to delete this room?')) {
			$.ajax({
				url: "{{ URL::route('backend.rooms.destroy', '') }}/" + id,
				type: "DELETE",
    ...
    function activateSelected() {
        ...
        if (ids.length > 0) {
            $('#formBulkActions').attr('action', "{{ URL::action('App\Controllers\Backend\RoomsController@activate') }}");
    ...
    function deactivateSelected() {
        ...
        if (ids.length > 0) {
            $('#formBulkActions').attr('action', "{{ URL::action('App\Controllers\Backend\RoomsController@deactivate') }}");
            $('#formBulkActions').find('input[type="hidden"]').val(ids);
            $('#formBulkActions').submit();
        }
    }
