			...
			<div class="col-sm-12">
				<h1 class="page-header">
					...
					<div class="pull-right">
						<a href="{{ URL::route('backend.hotels.create') }}" class="btn btn-primary"><i class="glyphicon glyphicon-plus"></i> Add</a>
					</div>
				</h1>
			</div>
        </div>
		<div class="row">
			<div class="col-sm-12">
				<div class="table-responsive">
					<table class="table table-striped table-bordered table-checkable" id="tableHotels">
						<thead>
							<tr>
                                <th>
							...
							<tr>
    							...
    							<td><input type="text" name="search_address" value="" class="form-control input-sm" placeholder="Search" autocomplete="off" /></td>
                                <td></td>
								<td><input type="text" name="search_phone" value="" class="form-control input-sm" placeholder="Search" autocomplete="off" /></td>
...
<script type="text/javascript">
	...
	$(document).ready(function() {
		table = $('#tableHotels').dataTable( {
			"sDom": "<'row dt-rt'<'col-sm-6'l>r>t<'row dt-rb'<'col-sm-6'i><'col-sm-6'p>>",
			"bProcessing": true,
			"bServerSide": true,
			"oLanguage": { "sSearch": "Search:" },
			"sAjaxSource": "{{ URL::route('backend.hotels.datatables') }}",
			"bSortCellsTop": true,
			"iDisplayLength": 50,
			"aaSorting": [
				[6,'desc']
	...
	function deleteItem(id) {
		if (confirm('Are you sure you want to delete this hotel?')) {
			$.ajax({
				url: "{{ URL::route('backend.hotels.destroy', '') }}/" + id,
				type: "DELETE",
    ...
    function activateSelected() {
        ...
        if (ids.length > 0) {
            $('#formBulkActions').attr('action', "{{ URL::action('App\Controllers\Backend\HotelsController@activate') }}");
    ...
    function deactivateSelected() {
        ...
        if (ids.length > 0) {
            $('#formBulkActions').attr('action', "{{ URL::action('App\Controllers\Backend\HotelsController@deactivate') }}");
            $('#formBulkActions').find('input[type="hidden"]').val(ids);
            $('#formBulkActions').submit();
        }
    }
