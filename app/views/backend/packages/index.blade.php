			...
			<div class="col-sm-12">
				<h1 class="page-header">
					...
					<div class="pull-right">
						<a href="{{ URL::route('backend.packages.create') }}" class="btn btn-primary"><i class="glyphicon glyphicon-plus"></i> Add</a>
					</div>
				</h1>
			</div>
        </div>
		<div class="row">
			<div class="col-sm-12">
				<div class="table-responsive">
					<table class="table table-striped table-bordered table-checkable" id="tablePackages">
						<thead>
							<tr>
                                <th>
							...
							<tr>
    							...
    							<td><input type="text" name="search_short_description" value="" class="form-control input-sm" placeholder="Search" autocomplete="off" /></td>
								...
								<td><input type="text" name="search_start_date" value="" class="form-control input-sm" placeholder="Search" autocomplete="off" /></td>
                                <td><input type="text" name="search_end_date" value="" class="form-control input-sm" placeholder="Search" autocomplete="off" /></td>
...
<script type="text/javascript">
	...
	$(document).ready(function() {
		table = $('#tablePackages').dataTable( {
			...
			"sAjaxSource": "{{ URL::route('backend.packages.datatables') }}",
			"bSortCellsTop": true,
			"iDisplayLength": 50,
			"aaSorting": [
				[6,'desc']
	...
	function deleteItem(id) {
		if (confirm('Are you sure you want to delete this hotel?')) {
			$.ajax({
				url: "{{ URL::route('backend.packages.destroy', '') }}/" + id,
				type: "DELETE",
    ...
    function activateSelected() {
        ...
        if (ids.length > 0) {
            $('#formBulkActions').attr('action', "{{ URL::action('App\Controllers\Backend\PackagesController@activate') }}");
    ...
    function deactivateSelected() {
        ...
        if (ids.length > 0) {
            $('#formBulkActions').attr('action', "{{ URL::action('App\Controllers\Backend\PackagesController@deactivate') }}");
            $('#formBulkActions').find('input[type="hidden"]').val(ids);
            $('#formBulkActions').submit();
        }
    }
