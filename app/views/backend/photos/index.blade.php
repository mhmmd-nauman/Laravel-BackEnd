...
@section('content')
	<div id="page-wrapper">
		<div class="row">
			<div class="col-sm-12">
				<h1 class="page-header">Photos <!-- <a href="{{ URL::route('backend.photos.create') }}" class="btn btn-primary pull-right"><i class="fa fa-plus"></i> Add photo</a> --></h1>
			</div>
        </div>
		<div class="row">
			<div class="col-sm-12">
				<div class="table-responsive">
					<table class="table table-striped table-bordered table-checkable" id="tablePhotos">
						<thead>
							<tr>
                                <th>
							...
							<tr>
                                ...
                                <td></td>
                                <td><input type="text" name="search_content_type" value="" class="form-control input-sm" placeholder="Search" autocomplete="off" /></td>
...
<script type="text/javascript">
	...
	$(document).ready(function() {
		table = $('#tablePhotos').dataTable( {
			"sDom": "<'row dt-rt'<'col-sm-6'l>r>t<'row dt-rb'<'col-sm-6'i><'col-sm-6'p>>",
			"bProcessing": true,
			"bServerSide": true,
			"oLanguage": { "sSearch": "Search:" },
			"sAjaxSource": "{{ URL::route('backend.photos.datatables') }}",
			"bSortCellsTop": true,
			"iDisplayLength": 50,
			"aaSorting": [
				[5,'desc']
	...
	function deleteItem(id) {
		if (confirm('Are you sure you want to delete this photo?')) {
			$.ajax({
				url: "{{ URL::route('backend.photos.destroy', '') }}/" + id,
				type: "DELETE",
    ...
    function activateSelected() {
        ...
        if (ids.length > 0) {
            $('#formBulkActions').attr('action', "{{ URL::action('App\Controllers\Backend\PhotosController@activate') }}");
    ...
    function deactivateSelected() {
        ...
        if (ids.length > 0) {
            $('#formBulkActions').attr('action', "{{ URL::action('App\Controllers\Backend\PhotosController@deactivate') }}");
            $('#formBulkActions').find('input[type="hidden"]').val(ids);
            $('#formBulkActions').submit();
        }
    }
